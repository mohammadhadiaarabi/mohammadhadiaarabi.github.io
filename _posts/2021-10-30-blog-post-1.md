...
We write the corresponding Poincaré map, that relates the phase when pulse at time t<sub>n</sub> arrives (&theta;(t<sub>n</sub>)) to the phase when the pulse at time t<sub>n + 1 </sub>arrives (&theta;(t<sub>n + 1</sub>)) (one can verify indeed indeed that &theta;(t<sub>n+1</sub>) = (&theta;(t<sub>n</sub>) + PRC(&theta;) + T<sub>s</sub>)mod T  = (&theta;(t<sub>new</sub>) + T<sub>s</sub>)mod T (&theta;(t<sub>n + 1</sub>)) for periodic stimulations with period T<sub>s</sub>). The following Poincaré phase map thus holds:
